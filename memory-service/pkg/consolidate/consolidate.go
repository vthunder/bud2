...
func NewConsolidator(g *graph.DB, llm LLMClient, inference *EdgeInference) *Consolidator {
	return &Consolidator{
		graph:               g,
		llm:                 llm,
		inference:            inference,
...
func (c *Consolidator) Run() (int, error) {
	...
	for {
		...
		if c.inference != nil && c.inference.verbose {
...
func (c *Consolidator) inferEpisodeEpisodeLinks(ctx context.Context, episodes []*graph.Episode) ([]EpisodeEdge, error) {
	...
	for start := 0; start < len(withSummaries); start += stepSize {
		...
		edges, err := c.inference.InferEpisodeEdges(ctx, episodesForInference)
		if err != nil {
