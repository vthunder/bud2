---
generated_at: 2026-04-11T02:40:00Z
commit: 524e027b
repomix: available
---
...
> Generated: 2026-04-11 | Commit: 524e027b

## Purpose

Bud is a personal AI agent that runs as a macOS launchd daemon (or Linux systemd service), providing autonomous assistance, long-term memory, and task management through a Discord interface. Built in Go, it uses a pluggable LLM provider architecture (`internal/executive/provider/`) to support multiple backends — Claude Code (`claude-code`), OpenCode Serve (`opencode-serve`), and OpenAI-compatible APIs — configured via `bud.yaml` (loaded by `internal/config/config.go`). The default provider is `claude-code`. Bud gives each LLM provider an event loop, attention system, and a rich set of MCP tools for interacting with external services and its own state.
...
| `internal/types/` | Shared type definitions — most-imported package (centrality 19) |
...
- `internal/config/config.go` — multi-provider LLM config: declares providers (claude-code, opencode-serve, openai-compatible), model role mapping, terminal manager; `bud.yaml.example` is the reference
- `internal/paths/paths.go` — `ResolveFile`/`MergeDir` implement the state-defaults overlay: `state/system/<path>` overrides `state-defaults/system/<path>` at read time; no files are copied on startup
...
- `bud.yaml.example` — reference config for LLM providers, model assignments, and terminal manager; copy to `bud.yaml` to customize
...
- **Entry points**: Single binary `bin/bud` built by `scripts/build.sh`. State server `bin/bud-state` is a separate MCP-only binary. `things-mcp` is a TypeScript server built separately. Runs as launchd (macOS) or systemd (Linux).
- **Patterns to know**: Core defaults (`bud`, `bud-ops` plugins, guides, reflexes, wakeup/startup instructions) live in `state-defaults/system/` — these are the shipping defaults. The `paths` package implements a read-time overlay: `state/system/<path>` overrides `state-defaults/system/<path>` when present; if not, the default is served directly without copying. This replaced the old seed-copy-on-startup model (commits 49aa775, 6efc99f). Config is loaded from `bud.yaml` (gitignored, specified via `--config` or `BUD_CONFIG`); `bud.yaml.example` is the reference. Terminal manager (zellij or tmux) is set via `terminal_manager:` in `bud.yaml`, defaulting to zellij. External plugins are declared in `state/system/plugins.yaml` — Bud clones/updates GitHub repos at startup into a local cache (auto-readonly), or loads from a `path:` local checkout. The `exclude:` list on a manifest entry skips named sub-plugin directories. At startup, `generateZettelLibraries` scans all plugin manifests for `"zettels"` declarations and writes `state/system/zettel-libraries.yaml`; manual entries (no `source:` field) are preserved across restarts. Agent definitions and skills hot-reload from plugins on every prompt without restart. The `state/` directory is Bud's working directory (`~/Documents/bud-state`) and is a separate git repo. Memory retrieval is disabled for startup impulses; the limit is 6 engrams for regular sessions. After `signal_done`, recalled memory quality ratings are sent back to Engram via `RateEngrams()`. Profiling is disabled by default — enable via `BUD_PROFILE=minimal|detailed|trace` (currently commented out, re-enable by uncommenting `main.go` block). OS-aware `STATE_PATH` defaults: `~/Documents/bud-state` (macOS), `$XDG_DATA_HOME/bud/state` or `~/.local/share/bud/state` (Linux).
...
- **Modifying reflex behavior**: `state-defaults/system/reflexes/*.yaml` — YAML rules evaluated by `internal/reflex/engine.go`; override in `state/system/reflexes/` for local customization (read via `paths.MergeDir`)
- **Changing how the LLM is prompted**: `internal/executive/executive_v2.go` around `buildPrompt` — context assembly is here; also check `state-defaults/system/core.md`, `state-defaults/system/wakeup.md` (autonomous wakes), and `state-defaults/system/startup-instructions.md` (daemon startup); override any of these in `state/system/`
...
- **Switching the terminal manager**: add `terminal_manager: tmux` to `bud.yaml` (default: `zellij`); see `internal/terminal/manager.go` for the interface
...
- **Understanding the overlay/defaults system**: `internal/paths/paths.go` — `ResolveFile` and `MergeDir` are the two entry points; understand how `state-defaults/` provides shipping defaults without copying files
